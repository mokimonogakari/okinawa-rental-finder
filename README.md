...
- **掲載媒体表示**: 各物件カードにソースサイト名をバッジ表示（どのサイトの物件か一目で判別）
- **沖縄特化の検索条件**: 台風対策の構造、駐車場、基地騒音等を含む詳細検索
- **保存済み条件クイック検索**: 通知条件として保存した検索をワンタップで呼び出し
- **AI適正価格推定**: 路線価・地価公示・周辺相場から機械学習モデルで適正賃料を推定
- **割安度スコア**: 実際賃料と推定賃料を比較し「お得物件」を自動判定
- **LINE新着通知**: 日次スクレイピングで新規取得した物件のうち、保存済み条件にマッチするものをLINE Messaging APIで自動通知
...
本アプリでは沖縄県を以下の4地域に分類し、さらに一部の市町村にはサブエリア（地域絞り込み）を設けています。

### 地域分類

| 地域 | 市町村 |
|------|--------|
| 南部 | 那覇市、豊見城市、南城市、糸満市、南風原町、与那原町、八重瀬町 |
| 中部 | 沖縄市、うるま市、宜野湾市、浦添市、北谷町、嘉手納町、読谷村、中城村、西原町、北中城村 |
| 北部 | 名護市、本部町、恩納村、金武町、宜野座村、国頭村、大宜味村、東村、今帰仁村 |
| 離島 | 宮古島市、石垣市、久米島町、竹富町、与那国町 |

### サブエリア定義（独自分類）

市町村内をさらに絞り込むための独自エリア定義です。物件の住所に含まれるキーワードでマッチングします。

#### 那覇市

| サブエリア | 住所キーワード | 説明 |
|-----------|---------------|------|
| 新都心エリア | 安謝、天久、おもろまち、銘苅、真嘉比、松島 | おもろまち駅周辺の商業・住宅地区 |
| 首里エリア | 首里 | 首里駅〜石嶺駅周辺の住宅地区 |

#### 浦添市

| サブエリア | 住所キーワード | 説明 |
|-----------|---------------|------|
| 経塚・前田エリア | 経塚、前田 | 経塚駅・浦添前田駅周辺 |
| 西原エリア | 西原 | てだこ浦西駅周辺 |

> サブエリアの定義は `config/search_conditions.yaml` の `sub_areas` セクションで管理しています。新しいエリアを追加する場合はこのファイルを編集してください。
...
| `config/search_conditions.yaml` | 検索条件マスタ (沖縄特化) |
